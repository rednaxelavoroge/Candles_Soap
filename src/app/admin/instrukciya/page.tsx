...
const CHAPTERS = [
  ...
  { id: "videoarhiv", num: "11", title: "Видеоархив: посмотреть, переименовать, удалить" },
  { id: "kontakty", num: "12", title: "Контакты и WhatsApp" },
  { id: "sohranit", num: "13", title: "Что происходит после «Сохранить»" },
  { id: "otkat", num: "14", title: "Если удалили лишнее" },
  { id: "melochi", num: "15", title: "Мелочи, которые стоит знать" },
];
  ...
  return (
    <div className="min-h-screen bg-bg">
      <div className="mx-auto flex max-w-2xl flex-col gap-12 px-5 py-12 md:px-8 md:py-16">
        ...
        <Chapter id="gde" num="02" title="Что где лежит">
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              ["Видео", "Архив всех роликов: посмотреть, переименовать, удалить, загрузить новый с телефона."],
          ...
          <Bullets
            items={[
              ...
              <>
                ...
                Файл берётся целиком, ничего заранее сжимать и обрезать не нужно: панель
                покажет, что ролик готовится, и через пару минут он появится в архиве. Ссылка
                на YouTube или Vimeo нужна только для чужих роликов: вставьте
                её в поле ниже и нажмите <Btn>Добавить</Btn>. Если это не ссылка на видео,
                панель так и скажет и ничего не прицепит.
              </>,
              <>
                <strong className="font-semibold">Длина ролика ничем не ограничена.</strong>{" "}
                Сколько сняли, столько и будет: ни панель, ни подготовка ролик не режут.
                Ограничение только по весу файла — 2 ГБ, а это больше любой съёмки
                с телефона. Ролики, которые лежали в архиве с самого начала («Свечи —
                процесс 1» и такие же), обрезаны до пятнадцати секунд: их так нарезали
                при первой загрузке съёмки. Целиком их можно только загрузить заново.
              </>,
            ]}
          />
          <h3 className="mt-2 font-display text-base font-semibold text-ink">
        ...
        <Chapter id="videoarhiv" num="11" title="Видеоархив: посмотреть, переименовать, удалить">
          <p className="text-[0.95rem] leading-relaxed text-ink">
            Вкладка <Btn>🎬 Видео</Btn> — все ролики мастерской в одном месте. Под каждым
            три кнопки: <Btn>▶ Смотреть</Btn> открывает ролик прямо в панели,{" "}
            <Btn>✏️</Btn> переписывает название, <Btn>🗑</Btn> удаляет ролик насовсем.
          </p>
          <Steps
            items={[
              <>Откройте вкладку <Btn>🎬 Видео</Btn></>,
              <>
                Найдите ролик — поиском по названию или кнопками <Btn>Свечи</Btn>,{" "}
                <Btn>Мыло</Btn>, <Btn>Другие</Btn>
              </>,
              <>Нажмите <Btn>🗑</Btn> под роликом</>,
              <>
                Панель спросит и покажет, где этот ролик сейчас стоит: в каких изделиях и
                в ленте «Бэкстейдж». Оттуда он тоже пропадёт
              </>,
              <>Нажмите <Btn>ОК</Btn></>,
            ]}
          />
          <p className="text-[0.95rem] leading-relaxed text-ink">
            С сайта ролик исчезает сразу, ждать выкладки не нужно. Из панели — тоже сразу,
            и после обновления страницы он не возвращается.
          </p>
          <Note label="Вернуть удалённое можно только через меня">
            Кнопки «отменить» в панели нет намеренно: удалить лишнее по ошибке слишком
            легко. Каждое действие остаётся в истории, поэтому вернуть можно любой день —
            но руками, по вашей просьбе.
          </Note>
          <p className="text-[0.95rem] leading-relaxed text-ink">
            Загрузить свой ролик — кнопка <Btn>➕ Загрузить видео</Btn> в правом верхнем
            углу этой же вкладки. У загруженного сразу появляется обложка: панель снимает
            кадр из самого ролика, пока он готовится.
          </p>
        </Chapter>

        <Chapter id="kontakty" num="12" title="Контакты и WhatsApp">
          <p className="text-[0.95rem] leading-relaxed text-ink">
        ...
        <Chapter id="sohranit" num="13" title="Что происходит после «Сохранить»">
          <p className="text-[0.95rem] leading-relaxed text-ink">
        ...
        <Chapter id="otkat" num="14" title="Если удалили лишнее">
          <p className="text-[0.95rem] leading-relaxed text-ink">
        ...
        <Chapter id="melochi" num="15" title="Мелочи, которые стоит знать">
          <Bullets
            items={[
              <>
