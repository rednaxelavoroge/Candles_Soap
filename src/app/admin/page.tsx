      ...
      */
      ...
      while (Date.now() < until) {
        ...
        if (state.state === "done" && state.src) {
          ...
          // «Готов» здесь было неправдой и стоило заказчице вечера. Обложку
          // панель кладёт прямо в папку сайта, поэтому картинка видна сразу,
          // а сам ролик к этой секунде лежит только в репозитории и приедет
          // выкладкой. Браузер в таком случае рисует ровно то, на что она
          // жаловалась 09.09.2026: картинка есть, внутри чёрное, Play серый.
          // Все прочие действия в панели честно пишут «через несколько минут»
          // — теперь пишет и это.
          showToast(
            `✓ Ролик принят${state.sizeMb ? ` — ${state.sizeMb} МБ вместо ${megabytes.toFixed(0)}` : ""}. На сайте появится через пару минут`,
          );
          return;
        }
