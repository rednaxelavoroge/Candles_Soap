...
export const productSchema = z.object({
  ...
  /**
   * Изделия, которые встанут внизу страницы в блоке «Похожие».
   *
   * Пустой список означает «выбери сама»: тогда соседи подбираются по
   * совпадению тем, как было до появления этого поля.
   */
  related: z.array(z.string()).optional(),
  /**
   * Позиция изделия внутри своей категории. Задаётся вручную из панели:
   * к сезону новогодние свечи поднимаются наверх, после праздника уходят вниз.
   * Без него порядок — тот, в каком изделия лежат в файле.
   */
  order: z.number().int().optional(),
});
